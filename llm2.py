...
当前迭代次数：{ITERATION}
探索时避免输出已知的结果，若5次探索以上未找到更小值，输出当前最小AoI的参数组合。
...
def call_llm_api(prompt):
    ...
    """
    client = OpenAI(
        api_key="sk-vVE4cgTz0JgJQWQGWBDJ8YCWL39N44b4A4NtkN460ppGXzRm",
...
def deduplicate_decisions(decisions):
    """
...
def main():
    ...
    for epoch in range(max_epochs):
        ...
        prompt = TASK_PROMPT.format(
            ...
            ITERATION=epoch,  # 使用 epoch 作为迭代计数
        ...
        # 去重并添加到历史决策
        unique_decisions = deduplicate_decisions(epoch_decisions)
        historical_decisions.extend(unique_decisions)

        # 保存去重后的决策到 history.txt
        with open(history_file, 'a', encoding='utf-8') as f:
            for decision in unique_decisions:
                f.write(decision + '\n')
        logger.info(f"Epoch {epoch + 1} 保存 {len(unique_decisions)} 条去重决策到 {history_file}")
